...
{
local ($in, $zinfo) = @_;
local $form = new AdFreeZoneUI::Form("save_zone.cgi");
$form->set_input($in);
$form->add_hidden("zone", $zinfo->{'name'});
local $section = new AdFreeZoneUI::Section($text{'edit_common'}, 4, undef, "100%");
...
local $auto = new AdFreeZoneUI::Select("autoboot", $zinfo->{'autoboot'},
				 [ [ "true", $text{'yes'} ],
                                   [ "false", $text{'no'} ] ]);
...
$form->add_button(new AdFreeZoneUI::Submit($text{'save'}, "save"));
$form->add_button_spacer();
foreach my $a (@actions) {
	$form->add_button(new AdFreeZoneUI::Submit($a->[1], $a->[0]));
	}
$form->add_button_spacer() if (@actions);
$form->add_button(new AdFreeZoneUI::Submit($text{'edit_delete'}, "delete"));
return $form;
}
...
{
local ($name, $value) = @_;
local $rv = new AdFreeZoneUI::OptTextbox($name, $value, 10, $text{'pool_none'});
...
local $p = new AdFreeZoneUI::ConfirmPage(&zone_title($zinfo->{'name'}),
		   $text{$action.'_title'},
...
local $p = new AdFreeZoneUI::Page(&zone_title($zinfo->{'name'}),
			    $text{$action.'_title'});
local $d = new AdFreeZoneUI::DynamicWait(\&execute_action, [ $action, $zinfo, $args ]);
$p->add_form($d);
...
local $form = new AdFreeZoneUI::Form("save_net.cgi", "post");
$form->add_hidden("zone", $zinfo->{'name'});
if ($net->{'address'}) {
...
$form->set_input($in);
local $section = new AdFreeZoneUI::Section($text{'net_header'}, 2);
$form->add_section($section);

local $ainput = new AdFreeZoneUI::Textbox("address", $address, 20);
...
local $ninput = new AdFreeZoneUI::OptTextbox("netmask", $netmask, 20,
				       $text{'default'});
...
if ($new) {
	$form->add_button(new AdFreeZoneUI::Submit($text{'create'}, "create"));
	}
else {
	$form->add_button(new AdFreeZoneUI::Submit($text{'save'}, "save"));
	$form->add_button(new AdFreeZoneUI::Submit($text{'delete'}, "delete"));
	}
return $form;
}
...
{
local ($name, $value) = @_;
return new AdFreeZoneUI::Select($name, $value,
...
local ($new);
local $form = new AdFreeZoneUI::Form("save_pkg.cgi", "post");
$form->set_input($in);
$form->add_hidden("zone", $zinfo->{'name'});
if ($pkg->{'dir'}) {
...
local $section = new AdFreeZoneUI::Section($text{'pkg_header'}, 2);
$form->add_section($section);

local $dinput = new AdFreeZoneUI::File("dir", $pkg->{'dir'}, 50, 1);
...
local ($in, $zinfo, $pkg) = @_;
local ($new);
local $form = new AdFreeZoneUI::Form("save_attr.cgi", "post");
$form->set_input($in);
$form->add_hidden("zone", $zinfo->{'name'});
if ($attr->{'name'}) {
...
local $section = new AdFreeZoneUI::Section($text{'attr_header'}, 2);
$form->add_section($section);

local $ninput = new AdFreeZoneUI::Textbox("name", $attr->{'name'}, 30);
$ninput->set_mandatory(1);
$ninput->set_validation_regexp('^\S+$', $text{'attr_ename'});
$section->add_input($text{'attr_name'}, $ninput);

local $tinput = new AdFreeZoneUI::Select("type", $attr->{'type'} || "string",
		       [ map { [ $_, $text{'attr_'.$_} ] }
			     &list_attr_types() ], 0, 1);
$section->add_input($text{'attr_type'}, $tinput);

local $vinput = new AdFreeZoneUI::Textbox("value", $attr->{'value'}, 30);
...
local ($new, $mount);
local $form = new AdFreeZoneUI::Form("save_fs.cgi", "post");
$form->set_input($in);
$form->add_hidden("zone", $zinfo->{'name'});
if ($fs->{'dir'}) {
...
local $section = new AdFreeZoneUI::Section($text{'fs_header'}, 2);
$form->add_section($section);
...
local $dinput = new AdFreeZoneUI::File("dir", $fs->{'dir'}, 50);
$dinput->set_mandatory(1);
...
if (&indexof($type, &mount::list_fstypes()) >= 0) {
	...
	local $sinput = new AdFreeZoneUI::Textbox("special", $fs->{'special'}, 40);
	$sinput->set_mandatory(1);
	$sinput->set_validation_func(\&validate_special);
	$section->add_input($text{'fs_special'}, $sinput);

	local $oinput = new AdFreeZoneUI::Textbox("options", $fs->{'options'}, 40);
...
local ($new, $mount);
local $form = new AdFreeZoneUI::Form("save_rctl.cgi", "post");
$form->set_input($in);
$form->add_hidden("zone", $zinfo->{'name'});
if ($rctl->{'name'}) {
...
local $section = new AdFreeZoneUI::Section($text{'rctl_header'}, 2);
$form->add_section($section);

local $ninput = new AdFreeZoneUI::Select("name", $rctl->{'name'},
                [ map { [ $_ ] } grep { /^zone\./ } &list_rctls() ],
                0, $in{'new'} ? 0 : 1);
$section->add_input($text{'rctl_name'}, $ninput);

local $table = new AdFreeZoneUI::InputTable([ $text{'rctl_priv'},
				        $text{'rctl_limit'},
				        $text{'rctl_action'} ]);
$form->add_section($table);
local $pinput = new AdFreeZoneUI::Select("priv", undef,
		   [ [ "", "&nbsp;" ],
		     [ "privileged", $text{'rctl_privileged'} ] ]);
local $linput = new AdFreeZoneUI::Textbox("limit", undef, 20);
$linput->set_mandatory(1);
$linput->set_validation_func(\&validate_limit);
local $ainput = new AdFreeZoneUI::Select("action", undef,
			   [ [ "none", $text{'rctl_none'} ],
			     [ "deny", $text{'rctl_deny'} ] ]);
...
{
local ($in) = @_;
local $form = new AdFreeZoneUI::Form("create_zone.cgi", "post");
$form->set_input($in);
local $section = new AdFreeZoneUI::Section($text{'create_header'}, 2);
$form->add_section($section);
&foreign_require("time", "time-lib.pl");

local $name = new AdFreeZoneUI::Textbox("name", undef, 20);
$name->set_mandatory(1);
$name->set_validation_func(\&validate_zone_name);
$section->add_input($text{'edit_name'}, $name);

local $path = new AdFreeZoneUI::OptTextbox("path", undef, 30,
		&text('create_auto', $config{'base_dir'}),
		$text{'create_sel'});
$path->set_validation_func(\&validate_zone_path);
$section->add_input($text{'create_path'}, $path);

local $brand = new AdFreeZoneUI::Select("brand",undef, [ &list_brands() ], 0, 0, $value ? 1 : 0);
$section->add_input($text{'create_brand'}, $brand);

local $address = new AdFreeZoneUI::OptTextbox("address", undef, 20,
...
local $install = new AdFreeZoneUI::Radios("install", 0, [ [ 1, $text{'yes'} ],
						    [ 0, $text{'no'} ] ]);
$section->add_input($text{'create_install'}, $install);

local $webmin = new AdFreeZoneUI::Radios("webmin", 0, [ [ 1, $text{'yes'} ],
						    [ 0, $text{'no'} ] ]);
$section->add_input($text{'create_webmin'}, $webmin);

local $inherit = new AdFreeZoneUI::Radios("inherit", 1, [ [ 1, $text{'pkg_inherit_yes'} ],
						    [ 0, $text{'pkg_inherit_no'} ] ]);
$section->add_input($text{'pkg_inherit'}, $inherit);


local $pkgs = new AdFreeZoneUI::Multiline("pkgs", undef, 5, 50);
$section->add_input($text{'create_pkgs'}, $pkgs);

$section->add_separator();

local $cfg = new AdFreeZoneUI::Radios("cfg", 1, [ [ 1, $text{'create_cfgyes'} ],
                                          [ 0, $text{'create_cfgno'} ] ]);
$section->add_input($text{'create_cfg'}, $cfg);

local $hostname = new AdFreeZoneUI::OptTextbox("hostname", undef, 20,
					 $text{'create_samehost'});
$hostname->set_validation_func(\&validate_zone_hostname);
$section->add_input($text{'create_hostname'}, $hostname);

local $root = new AdFreeZoneUI::OptTextbox("root", undef, 20,
					 $text{'create_same'});
$section->add_input($text{'create_root'}, $root);

local $currtz = &time::get_current_timezone();
local $timezone = new AdFreeZoneUI::OptTextbox("timezone", undef, 25,
...
local $locale = new AdFreeZoneUI::OptTextbox("locale", undef, 25,
					 &text('create_same2', $currlc));
$locale->set_validation_func(\&validate_zone_locale);
$section->add_input($text{'create_locale'}, $locale);

local $terminal = new AdFreeZoneUI::OptTextbox("terminal", undef, 25,
					 $text{'create_vt100'});
...
$resolv ||= "none";
local $rinput = new AdFreeZoneUI::Radios("resolv", $resolv,
...
local $domain = $resolv eq "dns" ? $dns->{'domain'}->[0] :
	  $resolv eq "nis" || $resolv eq "nis+" ? &net::get_domainname()
						: undef;
local $dinput = new AdFreeZoneUI::Textbox("domain", $domain, 20);
...
local $server = new AdFreeZoneUI::Textbox("server", $server, 40);
...
local $rinput = new AdFreeZoneUI::OptTextbox("router", $router, 20,
				       $text{'create_none'});
$rinput->set_validation_func(\&validate_zone_router);
$section->add_input($text{'create_router'}, $rinput);

$form->add_button(new AdFreeZoneUI::Submit($text{'create_ok'}));
return $form;
}
