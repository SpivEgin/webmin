...
$p = new AdFreeZoneUI::Page(undef, $module_info{'desc'}, "intro", 1, 1);
$zn = &get_current_zone();
if (!&has_command("zoneadm")) {
	...
	$form = new AdFreeZoneUI::Form();
	$form->set_input(\%in);
	$p->add_form($form);
	$table = new AdFreeZoneUI::Table([ $text{'list_name'},
                          $text{'list_id'},
                          $text{'list_path'},
                          $text{'list_status'},
	...
	foreach $z (@zones) {
		...
		foreach $a (&zone_status_actions($z)) {
			push(@actions, new AdFreeZoneUI::TableAction("save_zone.cgi", $a->[1], [ [ "zone", $z->{'name'} ], [ $a->[0], 1 ], [ "list", 1 ] ]));
			}
		$table->add_row([
