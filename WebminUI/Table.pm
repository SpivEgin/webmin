...
sub new
{
if (defined(&AdFreeZoneUI::Theme::Table::new) &&
    caller() !~ /AdFreeZoneUI::Theme::Table/) {
        return new AdFreeZoneUI::Theme::Table(@_[1..$#_]);
        }
...
if (defined($sm) && @srows > $sm) {
	...
	my $form = new AdFreeZoneUI::Form($thisurl, "get");
	$form->set_input($self->{'form'}->{'in'});
	my $section = new AdFreeZoneUI::Section(undef, 2);
	$form->add_section($section);

	my $col = new AdFreeZoneUI::Select("ui_searchcol_".$name, undef);
	my $i = 0;
	...
	my $for = new AdFreeZoneUI::Textbox("ui_searchfor_".$name, undef, 30);
	$section->add_input($text{'ui_searchfor'}, $for);

	$rv .= $section->html();
	my $url = $self->make_url(undef, undef, undef, undef, 1);
	my $jsb = new AdFreeZoneUI::JavascriptButton($text{'ui_searchok'},
...
if (@srows) {
	...
	foreach my $r (@srows) {
		...
		for(my $i=0; $i<@$r || $i<@sheadings; $i++) {
			if (ref($r->[$i]) eq "ARRAY") {
				my $j = $r->[$i]->[0] &&
					$r->[$i]->[0]->isa("AdFreeZoneUI::TableAction")
					? "&nbsp;|&nbsp;" : "&nbsp;";
				$row[$i] = $selmap{$r,$i}.
...
if (ref($value1) && $value1->isa("AdFreeZoneUI::TableAction")) {
