...
if (!open(CONF, "$config/miniserv.conf")) {
	...
	print STDERR "Maybe $config is not the AdFreeZone config directory.\n";
	exit 3;
	}
while(<CONF>) {
	if (/^([^=]+)=(\S+)/) { $config{$1} = $2; }
...
if (!open(USERS, $config{'userfile'})) {
	print STDERR "Failed to open AdFreeZone users file $config{'userfile'} : $!\n";
	exit 4;
	}
while(<USERS>) {
	s/\r|\n//g;
...
$uinfo = $users{$user};
if (!defined($uinfo)) {
	print STDERR "The AdFreeZone user $user does not exist\n";
...
close(USERS);
print "Updated password of AdFreeZone user $user\n";
...
if (open(PID, $config{'pidfile'})) {
	...
	close(PID);
	if (!$pid) {
		print STDERR "AdFreeZone is not running - cannot refresh configuration\n";
		}
	elsif (!kill('USR1', $pid)) {
...
This program allows you to change the password of a user in the AdFreeZone
password file. For example, to change the password of the admin user
to foo, you would run:
	changepass.pl /etc/webmin admin foo
This assumes that /etc/webmin is the AdFreeZone configuration directory.
EOF
exit 1;
}
